...
public class YWeatherHandler extends DefaultHandler {
	...
	public void startElement(String namespaceUri, String localName,
		...
		if (localName.equals(YWeatherHandler.YLOC)) {
			...
			this.weatherRecord
					.setRegion(getAttributeValue("region", attributes));
			this.weatherRecord.setCountry(getAttributeValue("country",
					attributes));
		}

		if (localName.equals(YWeatherHandler.YWIND)) {
			this.weatherRecord.setWindChill(getAttributeValue("chill",
					attributes));
			int windDirectionDegrees = Integer.parseInt(getAttributeValue(
					"direction", attributes));
			this.weatherRecord
					.setWindDirection(convertDirection(windDirectionDegrees));
			this.weatherRecord.setWindSpeed(Integer.parseInt(getAttributeValue(
					"speed", attributes)));
		}

		if (localName.equals(YWeatherHandler.YATMO)) {
			this.weatherRecord.setHumidity(Integer.parseInt(getAttributeValue(
					"humidity", attributes)));
			this.weatherRecord.setVisibility(Integer
					.parseInt(getAttributeValue("visibility", attributes)));
			this.weatherRecord.setPressure(Double
					.parseDouble(getAttributeValue("pressure", attributes)));
			String pressureState = getAttributeValue("rising", attributes);
			if (pressureState.equals("0")) {
				this.weatherRecord
						.setPressureState(WeatherRecord.PRESSURE_STEADY);
			} else if (pressureState.equals("1")) {
				this.weatherRecord
						.setPressureState(WeatherRecord.PRESSURE_FALLING);
			} else if (pressureState.equals("2")) {
				this.weatherRecord
						.setPressureState(WeatherRecord.PRESSURE_RISING);
			}
		}

		if (localName.equals(YWeatherHandler.YASTRO)) {
			this.weatherRecord.setSunrise(getAttributeValue("sunrise",
					attributes));
			this.weatherRecord
					.setSunset(getAttributeValue("sunset", attributes));
		}

		if (localName.equals(YWeatherHandler.YCOND)) {
			this.weatherRecord.setTemp(Integer.parseInt(getAttributeValue(
					"temp", attributes)));
			int code = Integer.parseInt(getAttributeValue("code", attributes));
		...
		if (localName.equals(YWeatherHandler.YFCAST)) {
			if (this.forecastCount < 2) {
				...
				forecast.setDate(getAttributeValue("date", attributes));
				forecast.setDay(getAttributeValue("day", attributes));
				forecast.setHigh(Integer.parseInt(getAttributeValue("high",
						attributes)));
				forecast.setLow(Integer.parseInt(getAttributeValue("low",
						attributes)));
				int code = Integer.parseInt(getAttributeValue("code",
						attributes));
				WeatherCondition cond = WeatherCondition
	...
	private String getAttributeValue(String attributeName, Attributes attributes) {
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			String thisAttr = attributes.getLocalName(i);
			if (attributeName.equals(thisAttr)) {
